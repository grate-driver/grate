...
bool framebuffer_save(struct framebuffer *framebuffer, const char *filename)
{
	...
	format = png_format(framebuffer->format);
	if (format == PNG_COLOR_TYPE_INVALID)
		return false;

	depth = png_depth(format);

	png_set_IHDR(png, info, framebuffer->width, framebuffer->height,
		     8, format, PNG_INTERLACE_NONE, PNG_COMPRESSION_TYPE_BASE,
		     PNG_FILTER_TYPE_BASE);
	png_write_info(png, info);
	...
	stride = framebuffer->width * (depth / 8);
	size = framebuffer->height * stride;
	...
	glReadPixels(0, 0, framebuffer->width, framebuffer->height,
		     framebuffer->format, GL_UNSIGNED_BYTE, data);
	glBindFramebuffer(GL_FRAMEBUFFER, 0);

	rows = malloc(framebuffer->height * sizeof(png_bytep));
	if (!rows) {
		fprintf(stderr, "out-of-memory\n");
		return false;
	}
